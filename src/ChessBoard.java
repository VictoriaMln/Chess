public class ChessBoard {
    ...
    public boolean moveToPosition(int startLine, int startColumn, int endLine, int endColumn){
        ...
        if (checkPos(startLine) && checkPos(startColumn) && //проверка, не уходят ли начальные и
                checkPos(endLine) && checkPos(endColumn)) { //конечные значения за края поля

            if (board[startLine][startColumn] == null) { //на начальной позиции есть фигура
                return false;
            }

            if (!nowPlayerColor().equals(board[startLine][startColumn].getColor())) { //проверка, что игрок соответствует фигуре
                return false;
            }

                //проверка возможности хода и сам ход
            if (board[startLine][startColumn].canMoveToPosition(this, startLine, startColumn, endLine, endColumn)) {
                board[endLine][endColumn] = board[startLine][startColumn];
                board[startLine][startColumn] = null;

                //проверка - ходили ли король и ладья
    ...
    public void printBoard(){
        ...
        //метод для проверки выхода за доску
        ...
        if (nowPlayer.equals("White")) {
            ...
            if (board[0][7].getSymbol().equals("R") && board[0][4].getSymbol().equals("K") &&
                ...
                if (board[0][7].getColor().equals("White") && board[0][4].getColor().equals("White") &&
                        board[0][7].getCheck() && board[0][4].getCheck() &&
            ...
            if (board[7][7].getSymbol().equals("R") && board[7][4].getSymbol().equals("K") &&
                ...
                if (board[7][7].getColor().equals("Black") && board[7][4].getColor().equals("Black") &&
                        board[7][7].getCheck() && board[7][4].getCheck() &&
